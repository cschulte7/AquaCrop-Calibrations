...
class SoilClass:
    ...
    **Attributes**:\n
    ...
    def __init__(self,soilType,dz=[0.1]*12,
        ...
        elif soilType == 'SandyLoam2018': # Clovis data from 2018
            self.CN = 46 
            self.CalcCN = 0
            self.REW = 7 
...
# 2018 only, pre-irrigation and full treatment
            ...
            self.MaxRooting = 1400 # Growing degree/Calendar days from sowing to maximum rooting -> end of development stage
            self.Senescence = 1400 # Growing degree/Calendar days from sowing to senescence
            self.Maturity = 1700 # Growing degree/Calendar days from sowing to maturity

# Check if percentage or decimal -> I think this is wrong. Should be days not percentage or decimal. Chose a random number tbh 
            self.HIstart = 880 # Growing degree/Calendar days from sowing to start of yield formation
            
            self.Flowering = 180 # Duration of flowering in growing degree/calendar days (-999 for non-fruit/grain crops)
...
# Check later 
            self.GDD_up = 4 # Minimum growing degree days (degC/day) required for full crop transpiration potential
            self.GDD_lo = 12.5 # Growing degree days (degC/day) at which no crop transpiration occurs
...
# They are sown 2-3 cm deep into the soil https://pfaf.org/user/Plant.aspx?LatinName=Cyamopsis and the seeds themselves are around 5 mm (.5 cm) long, then you have about 4.3 cm^2 surface area (using 2.5 cm height and 0.25 radius of the seed) -> SA of cylinder
            ...
            self.CCx = 1 # Maximum canopy cover (fraction of soil cover)
...
# Look into this 
            self.a_HI = 4 # Coefficient describing positive impact on harvest index of restricted vegetative growth during yield formation
            self.b_HI = 10 # Coefficient describing negative impact on harvest index of stomatal closure during yield formation
            self.dHI0 = 15 # Maximum allowable increase of harvest index above reference value
            
            self.Determinant = 0 # Crop Determinancy (0 = Indeterminant, 1 = Determinant)
            self.exc = 20 # Excess of potential fruits
...
# No water stress -> full irrigation, look into what these mean. See if there is a way to make it a compacted set and not a complex one. These are water stress factors. Can I get rid of them?           
            self.p_up1 = 0.70 # Upper soil water depletion threshold for water stress effects on canopy stomatal control
            self.p_up2 = 0.90 # Upper soil water depletion threshold for water stress effects on canopy stomatal control
            self.p_up3 = 0.98 # Upper soil water depletion threshold for water stress effects on canopy senescence
            self.p_up4 = 0.90 # Upper soil water depletion threshold for water stress effects on canopy pollination
            self.p_lo1 = 0.35 # Lower soil water depletion threshold for water stress effects on canopy expansion
